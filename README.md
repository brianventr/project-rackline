...
Cloudflare-native warehouse management for makers who grow into manufacturers. Iteration 1 covers organization tenancy, inventory in locations, inbound receipts, outbound pick/ship, cycle-count adjustments, BOMs, work orders, and Shopify order fulfillment.
...
- Click **Load Northwind Makers demo** (`demo@northwind.makers` / `rackline-demo`) to get a stocked shop: Desk Lamp BOM, bins `RECV` / `A-01-01` / `PROD` / `SHIP`, an open receipt, a floor order, Shopify order `#1004` (Maya Chen), and a work order.
...
- `operator` — floor actions (receive, pick, ship, complete WO, adjust) and Shopify order simulation. Cannot delete items, locations, or BOMs
