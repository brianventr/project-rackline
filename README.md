...
## New here: the tour and the setup wizard

Two guided flows teach the tool and its hierarchy without reading this file.

- **How Rackline works** is a short tour that opens once, on its own, for a new person: an owner on Today while the Getting started card is still open, an operator on the floor. Owners walk through the one-ledger idea, the hierarchy, the day's flow (inbound → stock → make → outbound), office vs floor, and Garage Mode vs Manufacturer; operators get the hierarchy and the floor (next job, scan, verbs). Reopen it any time from the display menu (the sliders icon in the top bar) → **How Rackline works**, from the command palette (⌘K → "How Rackline works"), or from the Getting started card. The seen flag is per person, per organization, per browser (`rackline.tour.seen:…`).
- **Set up your building** (`/welcome`, owners only, open in Garage Mode) builds the hierarchy for real: name the building and its timezone, add a dock, a rack of bays (aisle → rack → bay → level, with level 1 as the pick face and upper levels as bulk), a bench, and an outbound bay. A live tree and a rack front view update as you type, the review step shows the floor plan, and Create posts the same `/api/layout/racks`, `/api/layout/areas`, and `PATCH /api/locations/:id` calls Build floor uses, plus `PATCH /api/warehouses/:id` for the building's name and timezone. Codes are checked against every building, since barcodes are unique per organization. Steps whose bin already exists start switched off. The Getting started card's second step points here.

The hierarchy both flows teach, top to bottom: organization → warehouse (building) → area (dock, aisles, bench, outbound) → aisle → rack → bay → level → bin (the scannable location). What is there: SKU → on hand (one SKU in one bin) → overlays (lots, serials, expiry, catch-weight, holds, reservations). How it moves: document → ledger movement → floor job. The pure models live in `src/domain/hierarchy.ts`, `src/domain/tour.ts`, and `src/domain/setup-wizard.ts`; the glossary answers "what is a bay?" style questions in the palette.
