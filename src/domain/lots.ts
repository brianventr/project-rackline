...
export function allocateFifoLots(lots: LotRow[], qty: number, sku: string): LotRow[] {
  if (!Number.isInteger(qty) || qty <= 0) {
    throw new Error("Quantity must be a positive integer");
  }
  const sorted = [...lots]
    .filter((row) => row.qty > 0)
    .map((row) => ({ lotCode: row.lotCode.trim().toUpperCase(), qty: row.qty }))
  ...
  for (const row of sorted) {
    ...
    allocated.push({ lotCode: row.lotCode, qty: take });
    remaining -= take;
  }
  if (remaining > 0) {
